...
于是回退后的实际结果（"官"= 官方群，"旧"= 民间 bot 那个旧群）：

| 个人绑定 | 群绑定 | 属性/角色卡存的 key | 回退到官方主线后 |
|---|---|---|---|
| ✅ | ✅ | `旧群-旧号` | ✅ **完全正常**，和上游算出来的 key 一致 |
| ✅ | ❌ | `官群-旧号` | ⚠️ **孤儿**。上游按 `旧群-旧号` 查，读不到；`.pc list` 仍能看到卡（owner 是旧号） |
| ❌ | ✅ | `旧群-官号` | ⚠️ **孤儿**。上游按 `旧群-旧号` 查，读不到 |
