...
| **验证码验证身份归属** | `identityBindUseVerificationCode` | **`true`** | **防抢号的关键**，默认开启。详见 3.2.2 |
| 验证码位数 | `identityBindCodeLength` | `6` | 4~8 |
| 验证码有效期(秒) | `identityBindCodeExpireSec` | `600` | 60~3600，默认 10 分钟 |
| 优先用邮箱发验证码 | `identityBindPreferEmailCode` | `false` | 两条通道都可用时谁优先。默认私聊优先；打开后邮箱优先 |
...
> `identityBindFailCooldownSec`、`identityBindUseEmailCode`。答题路径整个去掉了
> （它挡不住抢号），邮箱通道也不再需要单独开关——**骰主把 SMTP 配好就等于要用它**。
> 残留的旧键会被 `Config.LoadYamlConfig` 静默忽略，不会报错。

### 3.2.2 验证码：两条通道 + 骰主人工兜底

**答题解决不了抢号问题。** 答题（角色卡名 / 日志名）只能拦住"完全不知道你信息的人"；
只要对方知道你的旧 QQ 号、又碰巧或猜到你的卡名，就能把你的整套数据
（角色卡、属性、日志）接管过去。

验证码解决的是**归属证明**：验证码只送到"只有那个旧号的主人才能拿到"的地方。
通道有两条，**自动二选一**，不需要骰主做额外开关：

| 通道 | 收件人 | 送到哪 | 谁来回复 |
|---|---|---|---|
| **邮箱** | 个人绑定 = 被声明的旧 QQ 号<br>群绑定 = 旧群的**邀请人** | `<QQ号>@qq.com` | **在官方 bot 这边**回复（群里、私聊都行） |
| **私聊** | 同上 | 民间 bot（OneBot）私聊 | 用那个号在**民间 bot** 那边回复 |

* 邮箱通道可用性 = `mailFrom` + `mailPassword` + `mailSmtp` 三项齐全（`CanSendMail()`）。
  地址是**我们数据库里的 QQ 号**推出来的，不接受用户自己填的邮箱——
  否则骰子就成了任意发信机。
* 默认**私聊优先**；打开 `identityBindPreferEmailCode` 后**邮箱优先**。
  先成功的那条算数，另一条失败也不会阻断绑定。
* 邮箱码**不要求回复者必须是发起人**：官方平台的"群内 OpenID"和"私聊 OpenID"
  不是同一个值（在群里发起、去私聊回复会被误判成"不是本人"），
  而群绑定的码本来就寄给邀请人、张罗绑定的常常是另一位群管理。
  真正证明身份的是"能不能拿到那封邮件"，而且绑定记录固定落在发起者名下。
* 私聊码**严格校验发信人**：码发给哪个号，就得哪个号回。那才是"持有旧号"的证明。
* **按目标号限频**：同一个旧号 / 旧群 10 分钟内最多发起一次验证码投递，
  全实例每小时最多 20 次（只在**真的发出去**了才计数）。
  这条是防"拿骰子当发信机"：否则任何人都能反复 `.bind <别人的号>`，
  消耗骰主的 SMTP 信誉（被投诉/被拉黑后所有通知邮件都废）与民间 bot 的私聊额度。
  用户主动 `.bind cancel` / `.group cancel` 会把这个额度放掉，不影响重试。
* 群绑定的回执里，邀请人的 QQ 号与邮箱是**打码**的（`243****084@qq.com`），
  避免在群里泄露第三方信息；个人绑定显示的是你自己刚填的号，保持原样。

**两条通道都不通时（没有民间 bot 在线、邮箱也没配）**：申请自动转人工——

* 私聊通知所有骰主（`QQ:<号>` 走民间 bot；`OpenQQ:<openid>` 走官方 bot；
  `UI:1001` 不是聊天账号会跳过），通知里写明申请内容与该敲哪条指令；
* 用户侧会明确告知"已通知骰主 / 没能通知到骰主"，骰主是发起人时还会直接给指令；
* 申请保留 **12 小时**（普通验证码只有 10 分钟）；
* 骰主用 `.bind pending` 看列表、`.bind approve <旧QQ号>` /
  `.group approve <旧群号>`（等价 `.group bindforce <旧群号>`）确认，
  **确认会跳过验证码，请先自行核实身份**。

**什么时候要关掉验证码**：只有在骰主既没有民间 bot、也没配邮箱，
且愿意接受抢号风险时才关。
...
- [ ] **验证码流程（邮箱通道）**：配好 SMTP（`mailFrom`/`mailPassword`/`mailSmtp`）后，
      用测试号 `.bind <旧QQ号>`，确认提示是「验证码已寄到 … 的 QQ 邮箱」，
      且**不会**再说"已通过民间 bot 发送了私聊验证码"；
- [ ] 打开那个 QQ 邮箱拿到验证码，**在官方 bot 这边**（群里或私聊都行）回复它，
      确认收到「验证通过」，官方群里也出现绑定成功通知；
- [ ] **验证码流程（私聊通道）**：把邮箱配置清空、让民间 bot 在线，再 `.bind <旧QQ号>`，
      确认改走民间 bot 私聊；用旧 QQ 号在民间 bot 那边回复验证码即可通过；
- [ ] **民间 bot 掉线时不许说谎**：只把 OneBot 连接停掉（配置里留着、State 不是已连接），
      再 `.bind <旧QQ号>`，确认**自动改用邮箱**，回复里不出现"民间 bot"字样；
- [ ] **SMTP 写错要报错**：故意把 SMTP 地址写错再 `.bind`，确认回复里带出 SMTP 错误，
      而不是"已寄出，请查收"；
- [ ] **没通道时转人工**：把邮箱配置清空、OneBot 也停掉，`.bind <旧QQ号>`，
      确认骰主收到私聊通知（`OpenQQ:` 的骰主走官方 bot、`QQ:` 的骰主走民间 bot），
      回复里说明"已通知骰主 / 没能通知到骰主"；
- [ ] **骰主兜底**：`.bind pending` 能列出申请，`.bind approve <旧QQ号>` 能直接绑定；
      在私聊里敲 `.group approve <旧群号>` 也要能用（不需要在那个群里）；
- [ ] **验证码过期**：发起后等超过 `identityBindCodeExpireSec`（默认 10 分钟）再回复，确认已失效。
- [ ] 绑定后 `.pc list` / `.st show` 能读到**旧角色卡**。
- [ ] 立刻再发起一次，确认出现"操作过于频繁"（冷却生效）。
- [ ] `.bind cancel` 能取消进行中的验证，取消后可以重新发起。
...
- [ ] `.bind cancel` 能取消进行中的验证，取消后可以重新发起。
- [ ] **群绑定的两条通道**：`.group bind <旧群号>` 后，确认验证码去了旧群**邀请人**
      （邮箱或民间 bot 私聊）；拿到码的人在官方 bot 这边回复也能过
      （不要求必须是发起的那位群管理）；`.log list` 能看到旧群日志。
- [ ] `.unbind` 之后 `.pc list` 恢复成绑定前的状态（证明没有动过数据）。
...
**Q：为什么回复验证码提示"这个验证码不是发给你的"？**
这条路只在**私聊通道**（民间 bot）上生效：码发给哪个旧 QQ 号，就得那个号回。
如果你走的是邮箱通道，回复的人不要求是发起人——出现这句说明挑战被当成了私聊码，
或者你回复的端点不对（邮箱码只在官方 bot 侧认，私聊码只在民间 bot 侧认）。
...
| （没设过 `.sn`，但绑定另一侧设过） | 沿用绑定另一侧的名片格式（迁移前设好的那份） |

> `.sn off` 与"没设过"是两回事：没设过会沿用绑定另一侧的模板，
> 而 `.sn off` 是**明确关掉**，会一直保持隐藏（记在 `identity-bindings.json` 里）。
> 想重新打开就再设一次 `.sn coc` / `.sn dnd` / `.sn expr`。

**状态栏里那个角色名取自"当前绑定的角色卡"**（`LoadByCtx`，与 `.st` 取属性同一个来源），
所以 `.pc tag <卡名>` 换卡之后名字会**立刻跟着变**，做过身份绑定的话两侧看到的是同一个。

> 为什么不直接用玩家名（`ctx.Player.Name`）：QQ 官方机器人**改不了群名片**
> （官方适配器的 `SetGroupCardName` 是空实现，这正是要做虚拟状态栏的原因），
> 所以那个字段在官方侧并不是实时更新的；再叠加身份绑定之后，
> 旧实现会优先取"另一侧（旧群）"的残留名片，于是换卡后状态栏还停在旧名字上。
> 现在的优先级是：当前卡名 → 玩家名 → 绑定另一侧的名字（兜底）。
...
如果关闭，官方 QQ 会按普通文本发送 —— 此时状态栏会**自动隐藏**，
（不会再往明文里塞 `$\scriptsize...$` 公式；早期版本会，属已知坑）。
