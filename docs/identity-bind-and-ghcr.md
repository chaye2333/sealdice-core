...
`.bind <旧QQ号> <旧群号>`。两个都做完之后，成员既能看到自己的角色卡，也能看到旧群的日志。
...
- [ ] **状态栏**：测试号发 `.sn coc`（或 `.sn dnd`），再发 `.r 1d20`，
      确认回复顶部出现**小号红字属性**，同一行右侧跟着角色名，角色名字号正常。
...
- [ ] `.group bind <旧群号>` 后 `.log list` 能看到旧群日志。
...
属性与角色名在**同一行**：属性是小号红字，角色名紧跟其后、保持正常字号。
...
* 属性部分是 QQ Markdown 的数学片段，客户端渲染成**小号红字**；
* 角色名放在片段之外，所以是**正常字号**，且与属性同行显示（更省版面）；
